...
let rec check env (expr : Cmm.expression) =
  ...
  | Ctrywith (body, _, handler) ->
    (* I believe that jumping from inside a trywith body to outside isn't
       very nice, but it's handled correctly by Linearize, as it happens
